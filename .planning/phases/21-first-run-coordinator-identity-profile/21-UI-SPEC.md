...
3. **Choose setup path.** `Save and continue` normalizes with `trim()` and caps at 48 characters, then advances without committing. Present a primary `Use recommended setup` action and a secondary `Advanced setup` action. Recommended copy states: `Save this coordinator securely and start it automatically.`
4. **Recommended password.** Show `Choose a coordinator password`, a password field, confirmation field, and `Save and start`. Both controls are at least 44px high, use browser password semantics, and never echo the secret in copy, logs, URLs, snapshots, or storage outside the encrypted key record. Empty and mismatched values use the coordinator store's existing exact errors. Successful submission enables persistence and autostart, commits setup, publishes the profile, and starts the coordinator in the same session.
5. **Advanced setup.** `Advanced setup` opens a four-step wizard with one decision surface at a time and a quiet `Advanced setup · N of 4` progress label. Steps are Persistence, Relays, Announcement, and Autostart. `Back` returns to the preceding advanced step without losing draft values; from Persistence it returns to the setup-path choice. `Continue` advances only when the current step is valid. The final action is `Finish setup` when autostart is off and `Save and start` when it is on.

The advanced Persistence step offers two full-row choices: `Persistent` (selected by default) and `Ephemeral`. Persistent expands passphrase and confirmation fields on that same step. The Relays step lists the current default relay URLs in editable inputs with labelled remove buttons, plus `Add relay`; rows accept only normalized `wss://` URLs and at least one valid enabled relay must remain. Announcement offers `No` (default) and `Yes`, describing that Yes makes the coordinator publicly discoverable. Autostart offers `Yes` (default) and `No`, describing whether the coordinator starts whenever this app opens.
...
- Keep name and all preference selections as an in-memory draft until final submission. Commit the relay set, announcement preference, autostart preference, normalized name, and completion marker together after any required encrypted key save succeeds.
- Recommended completion uses persistent identity, the displayed default relay set, announcements off, and autostart on. Advanced completion uses the values visibly confirmed in its four steps.
- The existing reset action clears setup completion and encrypted identity state; the next rendered state is the identity step of the full first-run flow.
...
- The path choice and binary-choice rows use the existing dark card vocabulary: a low-contrast border at rest, mint border/fill cue for the selected/default option, title plus one concise explanatory line, and no decorative animation. Relay rows stack at narrow widths without horizontal page scroll.
...
|-----------------|-------------------------------------------|
| Setup gate | `[data-testid="coordinator-setup"]` with `data-setup-state="checking|identity|required-name|preferences|saving|complete|migrated"`; it exists before any enabled lifecycle control when setup is not complete. |
...
| Setup path | `[data-testid="setup-path-stage"]`, `[data-testid="setup-recommended"]`, and `[data-testid="setup-advanced"]`; neither action commits setup before its flow finishes. |
| Recommended password | `[data-testid="setup-password-stage"]`, `[data-testid="setup-passphrase"]`, `[data-testid="setup-passphrase-confirmation"]`, `[data-testid="setup-persistence-error"]`, and `[data-testid="setup-save-and-start"]`. |
| Advanced persistence | `[data-testid="setup-advanced-persistence"]` with `data-step="1"`, plus `[data-testid="setup-persistent"]` and `[data-testid="setup-ephemeral"]`; Persistent is initially selected. |
| Advanced relays | `[data-testid="setup-advanced-relays"]` with `data-step="2"`, editable `[data-testid="setup-relay-input"]` rows, labelled remove controls, `[data-testid="setup-add-relay"]`, and inline validation. |
| Advanced announcement | `[data-testid="setup-advanced-announce"]` with `data-step="3"`; No is initially selected. |
| Advanced autostart | `[data-testid="setup-advanced-autostart"]` with `data-step="4"`; Yes is initially selected and the final action label reflects the choice. |

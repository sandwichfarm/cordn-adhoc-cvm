      ...
      <aside class="host-rail min-h-0 min-w-0 overflow-y-auto border-b border-[#21352a] p-3 sm:p-4 lg:border-r lg:border-b-0" data-testid="invite-panel">
        {#if coordinator.status !== "running"}
          <div class="space-y-6">
            ...
            <section class="space-y-3 border-t border-[#293832] pt-5"><div class="flex items-center justify-between"><h3 class="text-xs uppercase tracking-[0.16em] text-[#cfe2d4]">Runtime options</h3><span class="text-[10px] text-[#82958a]" data-testid="max-users-state">{config.maxUsers} key packages / identity</span></div><label class="flex items-center justify-between gap-4 text-sm text-[#cfe2d4]"><span>Announcement</span><input class="h-4 w-4 accent-[#7cf59d]" type="checkbox" checked={config.announce} disabled={!editingAllowed} aria-label="Toggle announcement" onchange={(event) => config.setAnnouncement(event.currentTarget.checked)} /></label><label class="grid gap-2 text-xs text-[#82958a]">Key-package quota<input class="host-input" type="number" min="1" max="256" step="1" value={config.maxUsers} disabled={!editingAllowed} aria-label="Key-package quota" data-testid="max-users-input" onchange={updateMaxUsers} /></label>{#if config.limitError}<p class="text-xs text-[#ffaaa3]" data-testid="limit-error">{config.limitError}</p>{/if}</section>
